  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            // Positioned with LOGICAL insets, not `left`/`right`. The element is
            // `fixed`, so physical offsets ignore `dir` entirely and would pin
            // the sidebar to the left of an Arabic page while the content flows
            // from the right -- overlapping it. `start`/`end` follow the
            // document direction, so one rule serves both LTR and RTL.
            "fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[inset-inline-start,inset-inline-end,width] duration-200 ease-linear lg:flex",
            side === "left"
